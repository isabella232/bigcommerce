...
import {
    ...
    PriceRecordBatchErrorSetFromJSON,
    PriceRecordBatchErrorSetFromJSONTyped,
    PriceRecordBatchErrorSetToJSON,
} from './';

/**
 * Errors during batch usage for the BigCommerce API.
 * @export
 * @interface PriceRecordBatchErrorResponse
 */
export interface PriceRecordBatchErrorResponse {
    /**
     * 
     * @type {Array<PriceRecordBatchErrorSet>}
     * @memberof PriceRecordBatchErrorResponse
     */
    batchErrors?: Array<PriceRecordBatchErrorSet>;
}

export function PriceRecordBatchErrorResponseFromJSON(json: any): PriceRecordBatchErrorResponse {
    return PriceRecordBatchErrorResponseFromJSONTyped(json, false);
}

export function PriceRecordBatchErrorResponseFromJSONTyped(json: any, ignoreDiscriminator: boolean): PriceRecordBatchErrorResponse {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'batchErrors': !exists(json, 'batch_errors') ? undefined : ((json['batch_errors'] as Array<any>).map(PriceRecordBatchErrorSetFromJSON)),
    };
}

export function PriceRecordBatchErrorResponseToJSON(value?: PriceRecordBatchErrorResponse | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'batch_errors': value.batchErrors === undefined ? undefined : ((value.batchErrors as Array<any>).map(PriceRecordBatchErrorSetToJSON)),
    };
}
