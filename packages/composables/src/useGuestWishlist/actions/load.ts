...
export const load: UseWishlistFactoryParams<
  Wishlist,
  WishlistItem,
  Product
>['load'] = async (context: Context) => {
  let guestWishlist: Wishlist = {
    ...
    name: context.$bigcommerce.config.app.$config.theme.wishlist.guest.name,
    customer_id: BIGCOMMERCE_GUEST_CUSTOMER_ID,
    items: [],
    wishlist_product_data: emptyProductsResponse,
    is_public: context.$bigcommerce.config.app.$config.theme.wishlist.isPublic,
  ...
  guestWishlist.wishlist_product_data = await refreshWishlistProducts(
    context,
    guestWishlist
  );
  window.localStorage.setItem(
